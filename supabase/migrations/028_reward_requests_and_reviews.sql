-- ============================================================================
-- 028 — Crea `reward_requests` y `public_reviews` (arregla "misiones").
--
-- La función de misiones / reseñas usa `public.reward_requests` (solicitudes de
-- recompensa CON estado y contactos bonus), que no existía → error
-- "Could not find the table 'public.reward_requests' in the schema cache".
--
-- Decisión: `customer_rewards` es un LEDGER de recompensas ya OTORGADAS
-- (reward_type, quantity...), no solicitudes pendientes. La tabla correcta es
-- `reward_requests` (status pending/approved/rejected + bonus_contact_ids), que
-- el código ya espera (MissionsSection, PublicReviews, AdminRewardsPage,
-- AdminRecompensasPage, CategoryDetailPage). Se crea junto a `public_reviews`
-- (las reseñas pueden originar una solicitud) y la vista `contact_trial_secure`
-- que las referencia. Idempotente; equivale a la migración 007.
-- ============================================================================
...
-- Vista segura de contactos de prueba/recompensa (referencia reward_requests).
create or replace view public.contact_trial_secure
with (security_barrier = true)
as
select
  c.id, c.category_id, c.name, c.description, c.phone, c.phone_masked,
  c.country_flag, c.country_code, c.tags, c.created_at
...
where c.status = 'active'
  ...
  and (
    exists (
      select 1 from public.trial_claims tc
      where tc.user_id = auth.uid() and c.id = any(tc.contact_ids)
    )
    or exists (
      select 1 from public.reward_requests rr
