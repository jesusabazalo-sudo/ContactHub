...
export default function QuickGuide({ mode = 'compact', forceOpen = false }: QuickGuideProps) {
  ...
  if (isHidden && !forceOpen) {
    return (
      <button
        ...
        className="focus-ring inline-flex items-center gap-2 rounded-full border border-brand-400/30 bg-surface px-4 py-2 text-sm font-bold text-brand-text shadow-[0_0_24px_rgba(29,180,122,0.08)] transition hover:bg-brand-400 hover:text-ink-950"
      >
  ...
  return (
    <section className="dopamine-card neon-edge rounded-3xl p-5 text-content sm:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="inline-flex items-center gap-2 rounded-full border border-brand-400/35 bg-brand-400/12 px-3 py-1 text-xs font-black uppercase tracking-[0.14em] text-brand-text">
            <ShieldCheck className="h-3.5 w-3.5" />
            Empieza por aquí
          </p>
          <h2 className="mt-4 font-display text-3xl font-bold leading-tight text-content">Bienvenido a ContactHub 👋</h2>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-content">
        ...
        {!forceOpen ? (
          <button
            ...
            className="focus-ring rounded-full border border-border bg-muted px-4 py-2 text-xs font-bold text-content transition hover:border-brand-400/45 hover:bg-brand-400/12"
          >
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {steps.map((step, index) => (
          <div key={step} className="card-hover rounded-2xl border border-brand-400/12 bg-surface/85 p-4 transition hover:border-brand-400/35 hover:bg-surface">
            <p className="stat-number text-xs font-black">{String(index + 1).padStart(2, '0')}</p>
            <p className="mt-2 text-sm font-semibold leading-5 text-content">{step}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        {guideCards.map((card) => {
          ...
          return (
            <article key={card.title} className="card-hover rounded-2xl border border-border bg-surface/90 p-4 transition hover:border-brand-400/35 hover:bg-surface">
              <Icon className="h-5 w-5 text-accent-cyan" />
              <h3 className="mt-3 text-sm font-bold text-content">{card.title}</h3>
              <p className="mt-2 min-h-12 text-xs leading-5 text-content-secondary">{card.text}</p>
              {card.to ? (
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-2">
          {infoBlocks.map((block) => (
            <article key={block.title} className="rounded-2xl border border-brand-400/20 bg-brand-400/[0.08] p-5">
              <h3 className="font-display text-lg font-bold text-content">{block.title}</h3>
              <p className="mt-2 text-sm leading-6 text-content">{block.text}</p>
            </article>
          ))}
        </div>
      ) : null}
