    ...
    <NavLink
      ...
      className={({ isActive }) =>
        `focus-ring inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold transition hover:-translate-y-0.5 ${
          isActive ? 'bg-gradient-to-r from-brand-400 to-accent-cyan text-ink-950 shadow-[0_0_22px_rgba(34,197,94,0.22)]' : 'text-content-secondary hover:bg-muted hover:text-content'
        }`
      }
    >
      <link.icon className="h-4 w-4" />
      {link.label}
      {badgeCount ? <span className="rounded-full bg-red-500 px-2 py-0.5 text-xs text-content">{badgeCount}</span> : null}
    </NavLink>
  );
}
    ...
    <section className="section-pad dopamine-surface bg-canvas">
      <div className="container-shell">
        <div className="mb-8 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase text-brand-text">Panel Admin</p>
            <h1 className="mt-3 font-display text-3xl font-bold text-content sm:text-4xl">ContactHub interno</h1>
          ...
          {user?.email ? <span className="inline-flex w-fit rounded-full border border-brand-400/25 bg-brand-400/10 px-4 py-2 text-sm font-semibold text-brand-text">Admin: {user.email}</span> : null}
        </div>
        ...
        <div className="mb-8 overflow-x-auto rounded-2xl border border-brand-400/15 bg-canvas/60 p-2 shadow-[0_0_28px_rgba(34,197,94,0.06)]">
          <div className="flex min-w-max items-center gap-2 whitespace-nowrap">
            <span className="px-3 text-xs font-bold uppercase tracking-wide text-content-muted">Gestion</span>
