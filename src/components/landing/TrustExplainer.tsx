    ...
    <>
      <section className="border-b border-border bg-canvas-subtle">
        <div className="container-shell grid gap-px bg-line sm:grid-cols-2 lg:grid-cols-5">
          {trustItems.map((item) => (
            <div key={item.label} className="flex min-h-20 items-center gap-3 bg-canvas-subtle px-4 py-4">
              <item.icon className="h-4 w-4 shrink-0 text-brand-text" />
              <span className="text-xs font-semibold leading-5 text-content-secondary">{item.label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="section-pad bg-canvas">
        <div className="container-shell grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
          <div>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              <Link to="/legal#privacidad" className="focus-ring inline-flex min-h-11 items-center justify-center rounded-lg border border-border bg-muted px-5 text-sm font-bold text-content transition hover:border-brand-400/35">
                Ver privacidad
              </Link>
              <Link to="/faq" className="focus-ring inline-flex min-h-11 items-center justify-center rounded-lg px-5 text-sm font-bold text-brand-text transition hover:bg-brand-400/[0.07]">
          ...
          <div className="grid gap-px overflow-hidden rounded-lg border border-border bg-line sm:grid-cols-2">
            {transparency.map((item, index) => (
              <div key={item} className="flex gap-3 bg-canvas-subtle p-5">
                <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-brand-text" />
                <div>
                  <span className="font-mono text-[11px] font-bold text-content-muted">0{index + 1}</span>
                  <p className="mt-1 text-sm leading-6 text-content-secondary">{item}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="container-shell mt-8">
          <div className="grid gap-3 rounded-lg border border-brand-400/15 bg-brand-400/[0.05] p-4 md:grid-cols-[auto_1fr] md:items-start">
            <MailCheck className="h-5 w-5 shrink-0 text-brand-text" />
            <div className="space-y-2 text-sm leading-6 text-content-secondary">
              <p>
                <strong className="text-content">Tu correo es tu llave de acceso.</strong> Lo usamos para identificar tu cuenta y proteger lo que desbloqueas.
              </p>
              <p>
