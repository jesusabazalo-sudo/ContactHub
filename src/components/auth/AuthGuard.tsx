        ...
        <div className="mx-auto grid min-h-[calc(100vh-80px)] max-w-2xl place-items-center">
          <div className="w-full rounded-3xl border border-brand-400/25 bg-[rgba(15,32,39,0.94)] p-6 text-center shadow-[0_24px_80px_rgba(0,0,0,0.38)] backdrop-blur-xl sm:p-8">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              ...
              <Link to="/catalogo" className="focus-ring inline-flex h-12 items-center justify-center rounded-2xl border border-border bg-muted px-5 text-sm font-bold text-content transition hover:border-brand-400/40">
                Explorar catálogo sin registrarme
              </Link>
            </div>
            <Link to={`/auth?redirect=${redirect}&mode=register`} state={{ from: location }} className="mt-4 inline-flex text-xs font-semibold text-brand-text hover:text-content">
              Prefiero crear cuenta con correo
            </Link>
            <p className="mt-5 text-xs leading-5 text-content-muted">Registro necesario solo para guardar tus accesos. Tu correo funciona como tu llave de entrada.</p>
          </div>
        </div>
      </div>
    );
